return {
   {
      ...
      config = function()
         ...
         local function setup_snippets()
            ...
            local sql_filetypes = { "sql", "plsql" }
            for _, ft in ipairs(sql_filetypes) do
               snip(ft, "log_dbms", "dbms_output.put_line('${1:message}: ' || ${2:variable});$0")
            ...
            snip("plsql", "cursor",
               "cursor ${1:c_name} is\n\tselect ${2:*}\n\tfrom ${3:table}\n\twhere ${4:condition};$0")
            snip("plsql", "forcursor", "for rec in ${1:cursor_name} loop\n\t${2:-- process rec}\nend loop;$0")
            snip("plsql", "forselect",
               "for rec in (select ${1:*} from ${2:table} where ${3:condition}) loop\n\t${4:-- process rec}\nend loop;$0")
            snip("plsql", "forloop", "for i in 1..${1:10} loop\n\t${2:-- process}\nend loop;$0")
            snip("plsql", "while", "while ${1:condition} loop\n\t${2:-- process}\nend loop;$0")
            snip("plsql", "loop", "loop\n\t${1:-- process}\n\texit when ${2:condition};\nend loop;$0")
            snip("plsql", "if",
               "if ${1:condition} then\n\t${2:-- true branch}\nelsif ${3:condition} then\n\t${4:-- elsif branch}\nelse\n\t${5:-- else branch}\nend if;$0")
            snip("plsql", "exception",
               "exception\n\twhen no_data_found then\n\t\t${1:-- handle}\n\twhen too_many_rows then\n\t\t${2:-- handle}\n\twhen others then\n\t\tca_log_pak.log_error('${3:source}', sqlerrm);\n\t\traise;$0")
            snip("plsql", "pragma", "pragma ${1:autonomous_transaction};$0")
            snip("plsql", "autonomoustx",
               "declare\n\tpragma autonomous_transaction;\nbegin\n\t${1:-- independent transaction}\n\tcommit;\nend;\n/$0")
            snip("plsql", "type", "type ${1:t_name} is ${2:table of varchar2(100)};$0")
            snip("plsql", "tabletype", "type ${1:t_tab} is table of ${2:table_name}%rowtype;$0")
            snip("plsql", "recordtype",
               "type ${1:t_rec} is record (\n\t${2:field1} ${3:varchar2(100)},\n\t${4:field2} ${5:number}\n);$0")
            snip("plsql", "pipelined",
               "create or replace function ${1:func_name} return ${2:t_table} pipelined is\nbegin\n\tfor rec in (select * from ${3:table}) loop\n\t\tpipe row(rec);\n\tend loop;\n\treturn;\nend;\n/$0")
            snip("plsql", "dynamicsql", "execute immediate '${1:sql statement}' ${2:using ${3:bind_vars}};$0")
            snip("plsql", "dbmsoutput", "dbms_output.put_line('${1:message}: ' || ${2:variable});$0")
            snip("plsql", "raiseapp", "raise_application_error(-20${1:001}, '${2:Error message}');$0")
            snip("plsql", "open", "open ${1:cursor_name};$0")
            snip("plsql", "fetch", "fetch ${1:cursor_name} into ${2:variable};$0")
            snip("plsql", "close", "close ${1:cursor_name};$0")
            snip("plsql", "returning", "returning ${1:column} into ${2:variable}$0")
            snip("plsql", "forall", "forall i in 1..${1:collection}.count\n\t${2:-- dml statement}$0")
            snip("plsql", "indices", "forall i in indices of ${1:collection}\n\t${2:-- dml statement}$0")
            snip("plsql", "values", "forall i in values of ${1:collection}\n\t${2:-- dml statement}$0")
            ...
            snip("markdown", "notes", "### NOTES: $0")
            snip("markdown", "todo-list",
               "## TODO\n\n- [ ] ${1:First task}\n- [ ] ${2:Second task}\n- [ ] ${3:Third task}\n$0")
            snip("markdown", "table",
               "| ${1:Header 1} | ${2:Header 2} | ${3:Header 3} |\n|------------|------------|------------|\n| ${4:Row 1} | ${5:Data} | ${6:Data} |\n| ${7:Row 2} | ${8:Data} | ${9:Data} |\n$0")
            snip("markdown", "details",
               "<details>\n<summary>${1:Click to expand}</summary>\n\n${2:Hidden content}\n\n</details>\n$0")
            snip("markdown", "badge",
               "![${1:Badge Name}](https://img.shields.io/badge/${2:label}-${3:message}-${4:color})")
            snip("markdown", "h1", "# ${1:Title}\n$0")
            snip("markdown", "h2", "## ${1:Section}\n$0")
            snip("markdown", "h3", "### ${1:Subsection}\n$0")
            ...
            snip("xml", "changeset",
               "<changeSet id=\"${1:id}\" author=\"${2:author}\">\n\t<sqlFile path=\"${3:file.sql}\" relativeToChangelogFile=\"true\"/>\n</changeSet>$0")
            snip("xml", "include", "<include file=\"${1:path/to/file.xml}\" relativeToChangelogFile=\"${2:true}\"/>$0")
            snip("xml", "includeall", "<includeAll path=\"${1:directory}/\" relativeToChangelogFile=\"${2:true}\"/>$0")
            snip("xml", "property", "<property name=\"${1:property_name}\" value=\"${2:value}\"/>$0")
            snip("xml", "precondition",
               "<preConditions onFail=\"${1:MARK_RAN}\">\n\t<sqlCheck expectedResult=\"${2:0}\">\n\t\tselect count(*) from ${3:table} where ${4:condition}\n\t</sqlCheck>\n</preConditions>$0")
            snip("xml", "context",
               "<changeSet id=\"${1:id}\" author=\"${2:author}\" context=\"${3:dev,test}\">\n\t${4:<!-- changes -->}\n</changeSet>$0")

            snip("sql", "lbchangeset",
               "--liquibase formatted sql\n\n--changeset ${1:author}:${2:id} runOnChange:${3:true} failOnError:true\n${4:-- sql statement}\n--rollback not supported$0")
            snip("sql", "lbchangesetplsql",
               "--liquibase formatted sql\n\n--changeset ${1:author}:${2:id} runOnChange:true failOnError:true endDelimiter:\"/\"\ncreate or replace ${3:procedure} ${4:name} as\nbegin\n\t${5:-- logic}\nend;\n/\n--rollback not supported$0")
            snip("sql", "lbprecondition",
            ...
            snip("sql", "lbformatted", "--liquibase formatted sql\n\n${1:-- changesets}$0")
         end
